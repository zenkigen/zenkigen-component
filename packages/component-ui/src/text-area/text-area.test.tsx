...
describe('TextArea', () => {
  ...
  describe('サイズバリエーション', () => {
    ...
    it('largeサイズのスタイルが適用されること', () => {
      ...
      expect(textarea.className).toMatch(/px-3/);
      expect(textarea.className).toMatch(/py-2/);
    });
  });
  ...
  describe('文字数カウンター', () => {
    describe('表示・非表示', () => {
      ...
      it('isCounterVisible={false}（デフォルト）でカウンターが非表示であること', () => {
        ...
        expect(screen.queryByText('3文字')).toBeNull();
      });
    });
    ...
    describe('エラースタイル', () => {
      it('counterMaxLength 超過時にエラースタイルが適用されること', () => {
        ...
        const counter = screen.getByText('5/3文字');
      ...
      it('maxLength 以内のときは通常スタイルであること', () => {
        ...
        const counter = screen.getByText('1/100文字');
      ...
      it('isError={true} でも上限未超過ならカウンターは通常スタイルであること', () => {
        ...
        const counter = screen.getByText('3/100文字');
        expect(counter.className).toMatch(/text-text02/);
        expect(counter.className).not.toMatch(/text-supportError/);
      });
    });
