...
export const TextInput = forwardRef<HTMLInputElement, Props>(
  ({ size = 'medium', isError = false, disabled = false, onClickClearButton, ...props }: Props, ref) => {
    const isShowClearButton = onClickClearButton && props.value.length !== 0 && !disabled ? true : false;
    ...
    return (
      <div className={inputWrapClasses}>
        ...
        {isShowClearButton && (
          <IconButton variant="text" icon="close" size="small" onClick={onClickClearButton} />
        )}
      </div>
    );
  },
