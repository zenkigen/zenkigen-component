...
export const iconComparisonEntries: IconComparisonEntry[] = [
  ...
  },
  {
    name: 'signal',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。lucide signal をそのまま使う（バー 5 本・accent なし）',
    candidates: ['signal'],
    hasAccent: false,
    tags: ['av'],
    issues: [],
  },
  {
    name: 'signal-low',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。※ lucide signal-low は「弱いバー 2 本だけ描いて残りを描かない」設計で現行と異なるため使わない。**lucide signal（5 本）をベースに低いバーへ class="accent" を付与して accent 対応する**（要 SVG 改変）',
    candidates: ['signal'],
    hasAccent: true,
    tags: ['av'],
    issues: [],
  },
  {
    name: 'signal-off',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。※ lucide signal-zero は「1 本だけ描く」設計なので使わない。**lucide signal（5 本）をベースに accent path を付与して accent 対応する**（要 SVG 改変）',
    candidates: ['signal'],
    hasAccent: true,
    tags: ['av'],
    issues: [],
