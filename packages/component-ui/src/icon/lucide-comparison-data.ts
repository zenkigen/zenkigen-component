...
export const iconComparisonEntries: IconComparisonEntry[] = [
  ...
  },
  {
    name: 'angle-left',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。単体は chevron-left と一致',
  ...
  },
  {
    name: 'angle-up',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。単体は chevron-up と一致',
  ...
  },
  {
    name: 'attention',
    status: 'keep',
    note: '状態アイコン（塗り円＋！）。Toast / NotificationInline の state アイコン 4 種をまとめて独自維持で確定（2026-09-08 ユーザー判断。2026-08-04 の置換確定を覆した）。候補は比較用に残す',
  ...
  },
  {
    name: 'graph-line',
    status: 'replace',
    note: '面塗りエリアチャート。塗りなしになる印象差を許容し chart-area で置換確定（2026-08-12 ユーザー判断。2026-07-30 の独自維持判断を覆した）',
  ...
  },
  {
    name: 'handle',
    status: 'replace',
    note: '置換確定（2026-08-12 ユーザー判断）。6 ドットのベタ塗り → grip-vertical。線画化の印象差を許容（論点 E から確定）',
  ...
  },
  {
    name: 'information',
    status: 'ask',
    note: '論点 E: information-filled とセット。filled 側は独自維持で確定（2026-09-08）したため同一化（論点 B）は解消。残る争点は、本件だけ lucide 化すると i マークの対（塗り／線）で描画スタイルが混在する点',
    candidates: ['info'],
    hasAccent: false,
    tags: ['status'],
    issues: ['E'],
  },
  {
    ...
    status: 'keep',
    note: '状態アイコン（塗り i）。state アイコン 4 種の独自維持確定（2026-09-08 ユーザー判断・論点 A 決着）。維持により information との同一化リスクは消滅。候補は比較用に残す',
    candidates: ['info'],
    hasAccent: false,
    tags: ['status'],
    issues: [],
  ...
  },
  {
    name: 'logout',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。現行は exit の左右鏡像で、置換すると exit と同じ log-out になり**向きの描き分けが消える**が許容の判断（2026-07-30 に一度維持側へ倒した判断を覆すもの）',
    candidates: ['log-out'],
    hasAccent: false,
    tags: ['auth'],
    issues: [],
  ...
  },
  {
    name: 'more',
    status: 'replace',
    note: '置換確定（2026-08-12 ユーザー判断）。横 3 点のベタ塗り → ellipsis。線画化の印象差を許容（論点 E から確定）',
    candidates: ['ellipsis'],
    hasAccent: false,
    tags: ['layout'],
    issues: [],
  ...
  },
  {
    name: 'score',
    status: 'ask',
    note: '論点 B: 文書＋テキスト 3 行＋右下にはみ出す大きなチェック。当初候補の file-check は upload-document-success に割当済みで同一化するため不可（2026-07-30 ユーザー判断）。2026-08-04 に lucide 1.25.0 の file 系 82 件を実査し file-check-2 を代替候補に。文書の右下が欠けチェックがはみ出す構造が現行に近く同一化も回避できる（file-check-corner は file-check-2 と同一 SVG のエイリアスなので候補に並べない）。ただし lucide に「テキスト行＋チェック」の複合は存在せずテキスト 3 行は失われる',
    candidates: ['file-check-2', 'file-badge'],
    hasAccent: false,
    tags: ['document', 'data'],
    issues: ['B'],
  },
  ...
  },
  {
    name: 'sidebar',
    status: 'replace',
    note: '置換確定（2026-08-12 ユーザー判断）。塗りパネル → panel-left。線画化の印象差を許容（論点 E から確定）',
  ...
  },
  {
    name: 'signal',
    status: 'keep',
    note: '独自維持で確定（2026-08-12 ユーザー判断。2026-08-04 の置換確定を覆した）。signal-low / signal-off と 3 点セットで現行 fill を維持。候補は比較用に残す',
    candidates: ['signal'],
    hasAccent: false,
    tags: ['av'],
    issues: [],
  },
  {
    name: 'signal-low',
    status: 'keep',
    note: '独自維持で確定（2026-08-12 ユーザー判断。2026-08-04 の「lucide signal ベース＋accent 付与」案を覆した）。現行 fill の accent 表現をそのまま維持。候補は比較用に残す',
    candidates: ['signal'],
    hasAccent: true,
    tags: ['av'],
    issues: [],
  ...
  },
  {
    name: 'success-filled',
    status: 'keep',
    note: '状態アイコン（塗り円＋✓）。state アイコン 4 種の独自維持確定（2026-09-08 ユーザー判断・論点 A 決着）。候補は比較用に残す',
  ...
  },
  {
    name: 'video',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。accent なし',
    candidates: ['video'],
    hasAccent: false,
    tags: ['av'],
    issues: [],
  ...
  },
  {
    name: 'volume',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。音波 2 本の volume-2 が現行相当（volume-1 は音波 1 本・volume は音波なし）。accent なし',
  ...
  },
  {
    name: 'volume-off',
    status: 'replace',
    note: '置換確定（2026-08-04 ユーザー判断）。**lucide volume-off の斜線 path（`m2 2 20 20`）に class="accent" を付与して accent 対応する**（要 SVG 改変）。現行も斜線が accent 色なので同じ表現を再現できる',
    candidates: ['volume-off'],
    hasAccent: true,
    tags: ['av'],
    issues: [],
  },
  {
    name: 'warning',
    status: 'keep',
    note: '状態アイコン（塗り三角＋！）。state アイコン 4 種の独自維持確定（2026-09-08 ユーザー判断。2026-08-04 の置換確定を覆した）。候補は比較用に残す',
