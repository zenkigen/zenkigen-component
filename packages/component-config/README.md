...
- `pnpm build:tokens` 実行時に以下が順に生成されます
...
| `style-dictionary/transformed-tokens.json` | 変換済みトークン  | `pnpm build:tokens` 内の `token-transformer` が生成。全ての参照が解決され HEX 等の生値を保持                                                                                  | Style Dictionary の入力。                                                                                                                                           |
