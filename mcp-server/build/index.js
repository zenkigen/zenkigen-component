...
server.tool('get_component_list', 'ZENKIGEN Component (ZDC) のコンポーネント一覧を取得する', {}, async () => {
    ...
    const componentList = componentNames.map((name) => components.entries[name].title.replace('Components/', '').replace('Tokens/Typography', ''));
    // componentList の重複排除
    const uniqueComponentList = [...new Set(componentList)];
    return {
        content: [{ type: 'text', text: uniqueComponentList.join('\n') }],
    };
});
// コンポーネントの詳細を取得するツール
server.tool('get_component_detail', '指定された ZENKIGEN Component (ZDC) のコンポーネント詳細情報を取得する', { name: z.string() }, async ({ name }) => {
