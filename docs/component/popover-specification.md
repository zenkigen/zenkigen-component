...
8. **ビューポート端での自動的な位置調整**
   - `placement`は希望する配置であり、ビューポートに収まらない場合は自動的に反転（`flip`）・ずらし（`shift`）が行われます
   - ビューポート端から8pxのマージンを確保するため、**端から8px以内に配置されたPopoverは最大8px内側へ移動します**
   - コンテンツがビューポートより大きい場合は収まりきらないため、その場合は表示サイズ側の調整が必要です
...
| 日付                 | 内容                                                                    | 担当者 |
| -------------------- | ----------------------------------------------------------------------- | ------ |
| 2026-08-06 11:16 JST | flip / shift ミドルウェアを追加（ビューポート端でのはみ出しを自動回避） | -      |
