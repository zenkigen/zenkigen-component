...
- `+N` バッジは表示中の Avatar の末尾（右端）に、最前面（最大の z-index）で配置される
...
### Q: z-index の重なり順を変えられる？（左の Avatar を前面にしたい）

A: 現在の実装では z-index は左から右に増加する固定仕様で、右側の Avatar が前面に重なる。重なり順を反転する API は提供していない。
