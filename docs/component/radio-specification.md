...
| `size`       | `'medium' \| 'large'`                        | `'medium'`   | ラジオボタンのサイズ。                                                   |
...
| 要素               | `medium`                    | `large`                     |
| ------------------ | --------------------------- | --------------------------- |
| クリック領域       | `size-6`（24px）            | `size-8`（32px）            |
| 視覚的な外枠       | `size-5`（20px）            | `size-7`（28px）            |
| 選択ドット         | `size-3`（12px）            | `size-4`（16px）            |
| ホバーインジケータ | `size-3`（12px）            | `size-4`（16px）            |
| ラベル             | `typography-label14regular` | `typography-label16regular` |
...
| 2026-04-06 16:34 JST | `size` prop（`large`）を追加 | -      |
