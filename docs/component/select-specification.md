...
- キーボード操作（オプションリスト）
  - トリガーを Enter / Space で押すとリストが開き、選択中のオプション（未選択なら先頭）へフォーカスが移る
  - `↑` / `↓` でオプション間を移動する（端ではループ）。`Home` / `End` で先頭・末尾へ
  - Enter / Space でフォーカス中のオプションを選択する
  - `Escape` / `Tab` でリストを閉じ、トリガーへフォーカスを戻す（`Escape` は外側へ伝播しない）
  - オプションの選択・選択解除でリストを閉じたときもトリガーへフォーカスを戻す。外側クリックで閉じた場合はフォーカスを移動しない
- オプションリストは `document.body` 直下にポータル描画されるため、トリガーから Tab で辿ることはできない（上記のキー操作で移動する）
- 選択状態が視覚的に明確に表示される
...
| 日付                 | 内容                                                                                                           | 担当者 |
| -------------------- | -------------------------------------------------------------------------------------------------------------- | ------ |
| 2026-09-14           | オプションリストのキーボード操作（矢印・Home/End・Escape/Tab）と、閉じたときのトリガーへのフォーカス復帰を追加 | -      |
