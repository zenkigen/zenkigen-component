...
オプションリストが表示される際、`selectedOption`に該当するアイテムが画面外にある場合、自動的にそのアイテムが見える位置までスクロールする。この機能は`optionListMaxHeight`が設定されている場合にのみ有効。

### 外部クリック検知

`useOutsideClick`カスタムフックを使用して、コンポーネント外部のクリックを検知し、オプションリストを自動的に閉じる機能を提供している。
