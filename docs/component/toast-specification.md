...
6. **モーダル表示中の通知**: `Modal` は表示中に背面の要素を `inert` にしますが、`ToastProvider` のポータルコンテナはライブラリ内部で除外指定されているため、モーダル表示中に発生した通知もポインタでの閉じる操作と読み上げが行えます（モーダル内の Tab 巡回には含まれないため、キーボードでは閉じるボタンに到達できません）。
...
| 2026-09-14           | Modal 表示中も操作・読み上げ可能である旨をアクセシビリティ節に追記                                                                                                                                                                                                                     | -      |
