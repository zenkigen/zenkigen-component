...
| `isAutoClose`    | `boolean`                                                           | `true`          | `true`にすると5秒後に自動で閉じ、`onClickClose`を呼び出します。`false`にすると自動では閉じません。                             |
...
| `description`    | `ReactNode`                                                         | `undefined`     | タイトルの下に表示する補足テキスト。省略した場合は説明文の要素自体が描画されません。                                           |
...
- `useToast` が返す `addToast` 関数は次の引数を受け取り、ユニークなIDでスタックに追加します。
...
- `isAutoClose` が `true`（既定）のとき、5,000ms 後に自動クローズが走ります。`isAnimation` が `true` の場合はフェードアウト完了後に、`false` の場合は即座に `onClickClose` が発火します。
...
1. `ToastProvider` のポータルコンテナに `role="status"` / `aria-live="polite"` / `aria-atomic="false"` を付与しており、追加されたトーストの内容がスクリーンリーダーに読み上げられます。live region はトースト追加より前からDOMに存在している必要があるため、`Toast` 側ではなくコンテナ側に置いています。**`Toast` を単体で使う場合は、利用側で live region を用意してください。**
...
4. 閉じるボタンを既定で非表示にしているのは、トーストが5秒で自動的に消えるため「早く消す」ためだけのボタンになるからです。手動で閉じさせたい場合のみ `hasCloseButton` を指定してください。
