...
| `counterMaxLength` | `number`  | -            | カウンター用の上限文字数（ソフトリミット）。超過時にカウンターがエラー色になるが入力はブロックしない |
| `maxLength`        | `number`  | -            | HTML ネイティブの最大文字数（ハードリミット）。入力をブロックする                                    |
...
| -------- | ---------------- | ---------------- | --------- | -------------- | ------------ |
| 従来     | `false`          | -                | 100       | なし           | あり         |
| ソフト   | `true`           | 2000             | -         | `14/2000`      | なし         |
| ハード   | `true`           | -                | 2000      | `14/2000`      | あり         |
| 表示のみ | `true`           | -                | -         | `14`           | なし         |
...
9. `counterMaxLength`はソフトリミットのため、HTML の`maxlength`属性は設定されず入力はブロックされません。`maxLength`はハードリミットとして HTML に反映されます。
