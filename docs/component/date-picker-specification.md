...
**モバイル向けのサイズ。トリガーボタンだけでなくカレンダーも拡大される**（342×437px）。他の3サイズはカレンダーが共通（224×295px）なのに対し、X-Largeのみカレンダーが専用の寸法になる。
...
> **X-Largeの利用条件**
>
> X-Largeのカレンダーは342×437pxで固定であり、スクロールしない。以下を満たすレイアウトでのみ使用すること。満たせない場合はX-Largeを使わず`large`以下を選ぶ。
>
> - ビューポート幅358px以上（カレンダー342px + 左右マージン8px×2）
> - カレンダー展開時、トリガーの**上または下のいずれか**に445px以上の余白（437px + offset 8px）が確保できること
>
...
| ------------------------ | ---------------------------------------------- | -------------------------------------------------- |
| カレンダー全体           | 224px × 295px                                  | **342px × 437px**                                  |
...
> react-day-pickerは`<table>`（`border-collapse: collapse`）で描画するためCSSの`gap`が使えない。Figmaの「40pxの行 + 8pxの間隔」は、**間隔を各行の高さに含める**ことで表現している（X-Largeの場合、48pxのセルに40pxのボタンを中央配置して上下左右に4pxずつの余白を作る）。曜日ヘッダーの行も同じ理由で48pxとし、その結果として月グリッドの下端には4pxだけが残る。
>
> 月ヘッダーはreact-day-pickerの`.rdp-month_caption { height: var(--rdp-nav-height) }`（border-box）により高さ40pxに固定され、`pb-2`は高さに加算されず内側に食い込む。このため40pxのナビボタンはcaptionの上へ4pxはみ出して置かれ、カレンダー上端からボタンまでは4px、ボタン下端から曜日行の視覚領域までは8pxとなる。
>
> この構造により、カレンダー上端から各行の中心までの距離はナビボタン24px / 曜日72px / 日付行120・168・216・264・312・360pxとなる。このレイアウトは**Figmaと重ねて確認済みで、実装を正とする**（実測: 342×437px）。
...
| 日付                 | 内容                                                                                                                      | 担当者 |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------ |
| 2026-08-18 16:45 JST | x-large カレンダーの寸法記載を実測値 342×437px に修正（実装レイアウトを正とする。月ヘッダーの height 固定の仕組みを追記） | -      |
