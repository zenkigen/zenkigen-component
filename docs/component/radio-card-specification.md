...
3. `label` / `description` は `string` のみ受け付ける。カード全面がクリック領域（透明な input のオーバーレイ）のため、カード内にリンクやボタン等の操作可能な要素を置く設計は意図的にサポートしていない。
4. カード内のテキストはマウスで選択・コピーできない（同オーバーレイ構造の既知の制約）。
