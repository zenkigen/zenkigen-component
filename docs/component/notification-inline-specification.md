...
| -------- | -------------------------- | ----------------------------- | ------------------ | ---------------------------------------- |
| `small`  | `typography-body13regular` | `calc(0.75rem - 1px)`（11px） | -                  | フィールドの中など高さを抑えたい場合。   |
...
`medium` はアイコン（24px）が 1 行時の高さの基準となるため、アイコンが表示されない `state="default"` でも高さが揃うよう `min-h-14` を適用している（`small` はテキストの行高がアイコン 16px を上回るため最小高さの指定は不要）。
