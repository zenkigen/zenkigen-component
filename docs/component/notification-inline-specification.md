...
| -------- | -------------------------- | ----------------------------- | ---------------------------------------- |
| `small`  | `typography-body13regular` | `calc(0.5rem - 1px)`（7px）   | フィールドの中など高さを抑えたい場合。   |
| `medium` | `typography-body13regular` | `calc(0.75rem - 1px)`（11px） | デフォルト。行間をやや広く取りたい場合。 |

パディングは常時付与される 1px の枠線（`variant` 参照）と合わせて、コンテンツから外形までのオフセットが従来どおり 8px / 12px になるよう設計されている。
