...
- アイコンとメッセージは上揃え（`items-start`）のグループにまとめられ、メッセージが複数行になってもアイコンは 1 行目の中央に揃う（`small` はアイコンコンテナを 1 行分の高さ `h-5` にして中央配置、`medium` はテキスト側に `pt-[2px]` を付けて 24px アイコンと 1 行目の中心を揃える）。アイコンの無い `state="default"` には `pt-[2px]` を付けず、テキストの中央位置が他 state と揃うようにしている。
