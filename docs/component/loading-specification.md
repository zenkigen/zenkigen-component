...
1. **待機時間が1秒未満の場合は表示しない**  
   画面の点滅を防ぎ、より滑らかな体験を提供します。
2. **繰り返し操作の抑止に利用**  
   ユーザーの連打を防ぐため、フォーム送信後すぐにLoadingをオーバーレイ表示します。
3. **進捗率が明示できる場合はProgressコンポーネントへ切り替え**  
   長時間待機や進捗が算出できるケースでは確定型のUIを優先してください。
4. **色は親要素の`color`に従う**  
   ダーク背景やブランド配色で使用する際は、ラッパーに `text-textOnColor` など適切なカラークラスを付与します。
