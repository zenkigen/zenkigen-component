...
| プロパティ           | 型                                              | デフォルト値 | 説明                                                                 |
| -------------------- | ----------------------------------------------- | ------------ | -------------------------------------------------------------------- |
| `size`               | `'medium' \| 'large' \| 'x-large'`              | `'medium'`   | コンポーネントのサイズを指定する                                     |
...
- `large`
  ...
  - ヘルパー/エラーメッセージ: `typography-label13regular`
- `x-large`
  - タイポグラフィ: `typography-label16regular`
  - 高さ: `min-h-12`
  - パディング: `px-3`（`variant="outline"` のみ。`variant="text"` ではパディングなし）
...
| 入力パディング        | `px-2`（medium）/ `px-3`（large・x-large） | なし                                   |
...
| クリアボタン右余白    | `pr-2`（medium）/ `pr-3`（large・x-large） | なし                                   |
...
- クリアボタン表示時（Outline バリアント）、コンテナに右側パディングを付与する（`pr-2`: medium / `pr-3`: large・x-large）。入力側は `pr-0` となる
...
// x-large
<TextInput value={value} size="x-large" onChange={(e) => setValue(e.target.value)} />
```
...
| 日付                 | 内容                                                               | 担当者 |
| -------------------- | ------------------------------------------------------------------ | ------ |
| 2026-05-19 00:00 JST | `size` に `'x-large'` を追加（高さ 48px / 16px フォント）          | -      |
