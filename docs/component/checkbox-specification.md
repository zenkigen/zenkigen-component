...
| `size`            | `'medium' \| 'large'`                        | `'medium'`   | サイズバリエーション。`medium` は box 20px / ラベル 14px、`large` は box 24px / ラベル 16px。 |
...
- **サイズ**: `size` で視覚ボックスとラベルの寸法が切り替わる（`'medium'`: box 20px + ラベル 14px / `'large'`: box 24px + ラベル 16px）。詳細は「状態とスタイル / サイズバリエーション」を参照。
...
- **medium（デフォルト）**: 視覚ボックス 20×20px (`size-5`)、外側ラッパー 24×24px (`size-6`)、ラベル `typography-label14regular`（14px）、ホバーインジケータ 12×12px (`size-3`)。チェック／マイナスアイコンは 20×20px で描画される。
- **large**: 視覚ボックス 24×24px (`size-6`)、外側ラッパー 32×28px (`h-8 w-7`, 高さは `typography-label16regular` の行高さに合わせて設定)、ラベル `typography-label16regular`（16px）、ホバーインジケータ 16×16px (`size-4`)。チェック／マイナスアイコンは 24×24px に拡大される。
- **hit area**: いずれのサイズでも透明な `input` を外側ラッパー全域に `absolute inset-0` で敷き、クリック／ホバー判定を外側ラッパーと一致させる。これにより `large` の非対称ラッパーでも dead zone が生じない。

### その他のスタイル仕様

- ラベル: サイズに応じて `typography-label14regular`（medium）または `typography-label16regular`（large）を使用し、`label` が指定されたときのみ `htmlFor={id}` 付きで右側に表示する。
...
| 2026-04-08 JST       | `size` prop（`'medium' \| 'large'`）を追加 | -      |
