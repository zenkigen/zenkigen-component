...
`portalTargetRef` には `null` 初期化した `useRef` を渡し、メニューを任意のレイヤーに移動させる。ターゲットDOMは描画時点で存在する必要がある。ポータル先にはメニューを包むラッパー要素（`div`）が常時描画される。

`Modal` の中で `portalTargetRef` を使う場合、ポータル先は `Modal` の DOM 配下を指すこと（`Modal` は表示中に背面の要素を `inert` にする。ラッパーは除外されるが、`z-dropdown` は `Modal` のオーバーレイより低いため、`Modal` の外に描画すると重なり順が保証されない）。
...
- アイテムを選択するとメニューを閉じ、フォーカスをトリガーに戻す（アイテムの `onClick` はフォーカスを戻した後に呼ばれるため、`onClick` で `Modal` を開くと閉じたときにトリガーへ戻れる）。
- `useOutsideClick` によりメニュー外をクリックすると自動で閉じる。このときフォーカスは移動しない（クリック先から奪わない）。
...
5. `Modal` の中で `portalTargetRef` を使う場合、ポータル先は `Modal` の DOM 配下を指すこと（「ポータル描画例」参照）。
...
| 日付                 | 内容                                                                                                          | 担当者 |
| -------------------- | ------------------------------------------------------------------------------------------------------------- | ------ |
| 2026-09-14           | アイテム選択後のトリガーへのフォーカス復帰、`portalTargetRef` 指定時のラッパー要素と `Modal` 内での推奨を追記 | -      |
