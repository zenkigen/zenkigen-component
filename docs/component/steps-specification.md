...
| `defaultCurrentStep` | `number`                         | `0`            | `currentStep` 未指定時の初期表示値          |
...
2. `currentStep` と `defaultCurrentStep` が両方渡された場合は `currentStep`（controlled）を優先する。なお `defaultCurrentStep` は「`currentStep` 未指定時の初期表示値」としてのみ機能し、内部で変化する setter は提供していない
