...
`label` には文字列以外に inline-level な ReactNode（`<span>`, `<Tag>`, `<Icon aria-hidden>`, Fragment 等）を渡せる。詳細は「注意事項」を参照。
...
6. **`label` に渡せる ReactNode の制約**
   - `label` は `<span>` の中に差し込まれる前提のため、**inline-level な ReactNode のみ**を渡すこと（テキスト、`<span>`, `<Tag>`, `<Icon>`, Fragment 等）
