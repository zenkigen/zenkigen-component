...
- `getInsideElements` で以下を `inert` の対象から除外する
  ...
  - ライブラリ内部で除外指定した要素（Toast のコンテナ）— Toast はモーダルより前面に出る設計で、表示中もポインタ操作・読み上げ可能に保つため（モーダル内の Tab 巡回には含まれない）
  - floating-ui 自身も `[aria-live]` / `[role="status"]` / `output` を自動で除外する
- 開く直前にフォーカスがあった要素をモーダルごとに保持し `returnFocus` に渡す（入れ子のモーダルを内側→外側の順に閉じても、最初に開いた要素へ戻すため）
...
6. **背面要素の `inert`**: モーダル表示中、モーダルより前から DOM に存在していた背面の要素は `inert` になり、クリックもキーボード操作も支援技術からの読み上げもできなくなる（走査は `document.body` 直下だけでなく、除外要素の祖先の配下にも再帰する）。除外されるのは、モーダルより後に追加された floating-ui のポータルの器と別のモーダル、ライブラリ内部で除外指定した要素（Toast のコンテナ。ポインタ操作と読み上げは可能だが Tab では到達しない）、および floating-ui が自動で除外するライブリージョン（`aria-live` / `role="status"` / `output`）。この判定はモーダルを開いた瞬間に一度だけ行われる。
7. **モーダル内で使うポップアップ**: 本ライブラリのコンポーネント（Popover / DatePicker / Combobox / Select / Dropdown）はモーダル内で表示・マウス操作できる。ただし Select / Dropdown の候補リストはキーボード（Tab）では到達できない（矢印キーによる操作は今後の対応課題）。自前実装のポップアップをモーダルの DOM 配下ではない場所へポータル描画する場合、ポータル先がモーダル表示前から存在していると `inert` になり操作できなくなる。自前のポップアップはモーダルの DOM 配下に描画するか、開いたときにポータル先を生成すること。`Dropdown` の `portalTargetRef` はモーダルの DOM 配下を指すのが推奨。
8. **複数のモーダル**: 順番に開いた場合は後から開いたモーダルが前面になり、前のモーダルは `inert` になる。同時に（同じレンダーで）開いた場合は DOM 上で後ろにあるモーダルが前面として扱われる。内側のモーダルを `portalTargetRef` で外側のモーダルの外（`inert` になったコンテナの配下）へ描画すると操作できないため、入れ子では `portalTargetRef` を指定しないこと。
...
| 2026-09-14 | 複数モーダル・背面のポップアップの `inert` の扱い、入れ子を閉じたときのフォーカス復帰、Toast の操作範囲を注意事項に追記                                | -      |
