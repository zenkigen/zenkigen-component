...
| バージョン | 対象  | 変更内容                                                                                          | 判断の根拠                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ----- | ------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 未リリース | Toast | `ToastProvider` 経由のトーストの閉じるボタンを既定で非表示に変更（`hasCloseButton` 既定 `false`） | 社内 10 アプリ・500 箇所超の利用を調査した結果、すべて `ToastProvider` 経由で `addToast({ message, state })` のみを使用しており、既存の呼び出しは無変更で動作する。変わるのは見た目（閉じるボタンの有無・枠線・角丸）のみで、VRT を運用しているアプリには差分が出るため告知に明記した。なお調査範囲は社内アプリのみで公開 npm の社外利用者は把握できないため、素の `Toast` の既定値（`isAutoClose: false`）は変更せず据え置き、API 挙動の非互換が生じないようにした（新しい既定は Provider が `isAutoClose` を明示的に渡すことで実現） |
