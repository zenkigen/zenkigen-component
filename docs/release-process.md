...
リリースは **`v*` タグの push を起点に GitHub Actions が自動で npm publish する**フローで運用している。ローカルで `pnpm publish:all` を手で叩く運用ではない（手動コマンドは緊急時のフォールバック）。

```
version bump（4 package.json のみ / pnpm-lock.yaml は変化しない）
        ↓ commit（release: X.Y.Z）
vX.Y.Z タグを作成・push
        ↓ publish.yaml が発火（on: push: tags: 'v*'）
CI: pnpm install → pnpm build-lib:all → pnpm publish:all --tag <dist-tag>
        ↓
...
ルートの `zenkigen-component`（private なワークスペース）は publish 対象外（`pnpm -r` はワークスペースルートを対象に含めない）。

## バージョン同期ルール

- **全パッケージの version を常に揃える**（部分的なバージョン更新はしない）。
- 内部依存（`@zenkigen-inc/*`）は **`workspace:*`** で参照している。pnpm が publish 時にその時点の version の exact 値へ自動変換する（`"workspace:*"` → `"1.22.1"`）ため、利用者が見る公開物は exact 固定になる。
- **version bump で `pnpm-lock.yaml` は変化しない。** リリースコミットに含めるのは 4 つの `package.json` だけ。

### なぜ lockfile が変わらないのか

`pnpm-lock.yaml` は内部依存を、バージョンではなくディレクトリへのリンクとして記録している。

```yaml
packages/component-ui:
  dependencies:
    '@zenkigen-inc/component-config':
      specifier: workspace:* # ← 範囲を持たない
      version: link:../component-config # ← package.json の version を参照していない
```
...
3. **`pnpm install --frozen-lockfile` を実行する**
   ...
   - `pnpm-lock.yaml` に差分が出たら異常。commit せず中断して調査する（[lockfile に差分が出た場合](#lockfile-に差分が出た場合)）。
...
7. **CI の publish 完了を待つ**
   ...
   - CI の中身: `pnpm install --frozen-lockfile` → `pnpm build-lib:all` → `pnpm publish:all --tag <dist-tag> --no-git-checks`。
   - `publish:all` = `pnpm -r publish`。`--no-git-checks` はタグからの checkout が detached HEAD になり、
     publish ブランチの判定に掛かるため付けている。`--tag` は CI がタグ名から判定（安定版 → `latest` / プレリリース → `next`）。
...
リリース作業で `pnpm-lock.yaml` が変化するのは正常ではない。**差分をそのまま commit しないこと。**
...
| 第三者パッケージのバージョン・integrity | **中断**。誰がいつ入れた変更か特定できるまで進めない                                  |
...
> 依存を更新する PR では、導入するバージョンの公開日・provenance・既知のサプライチェーン攻撃の対象かどうかを確認する。`pnpm-workspace.yaml` の `minimumReleaseAge: 4320`（公開 3 日未満をブロック）と、pnpm が依存のビルドスクリプトを既定で実行しないこと（許可は `allowBuilds` の allowlist のみ）が一次防御として効いている。

### 具体例: 1.22.0 → 1.22.1 をリリースする

人が手を動かすのはステップ 1〜6（ブランチ切り替え → `version` 行の書き換え → `pnpm install --frozen-lockfile` → commit → push → タグ push）だけ。ビルドや publish コマンドは打たない（CI が実施する）。
...
```bash
pnpm install --frozen-lockfile               # lockfile が変わらないことの確認（CI と同条件）
...
- ⚠️ `pnpm-workspace.yaml` の `minimumReleaseAge: 4320` が効く環境では、公開直後3日間はそのバージョンの install がブロックされる（publish には無影響）。プレリリース検証時はこの待ち時間を考慮する。
...
- `pnpm-workspace.yaml`:
  - `registries.default: https://npm.flatt.tech/` … install を Takumi Guard 経由に固定する（publish 先ではない）。
  - `minimumReleaseAge: 4320` … リリースから 3 日未満のバージョンは **install をブロック**（publish には影響しない）。
  - `allowBuilds` … 依存のビルドスクリプトの allowlist。pnpm は既定で実行しないため、ここにあるものだけが走る。
  - `saveExact: true` … `pnpm add` 時のバージョンを exact 固定にする（`workspace:*` の publish 時変換結果も exact になる）。
- 各 `package.json` の `publishConfig`:
  - `access: public`。
  - `registry: 'https://registry.npmjs.org/'` … **既定レジストリが Takumi Guard のため、これが無いと publish 先がプロキシになる。**
- パッケージマネージャ: `pnpm@12.4.0`。Node と pnpm のバージョンの源は
  ルート `package.json` の `packageManager` / `engines` と `.node-version` の 3 つだけ。
- **dist-tag**: タグ名から自動判定する（[npm dist-tag の判定ルール](#npm-dist-tag-の判定ルール)）。安定版（ハイフンなし）は `latest`、プレリリース（ハイフンあり）は `next`。`publish:all` には CI が `--tag <判定結果> --no-git-checks` を付けて実行する。
  - **公開済みバージョンのスキップは pnpm の標準挙動**（`pnpm -r publish` は「まだ公開されていないバージョン」だけを publish する）。yarn 時代の `--tolerate-republish` に相当するフラグは不要になった。

## よくある落とし穴

- **pnpm-lock.yaml を巻き込んで commit する**: version bump では lockfile は変化しない（[なぜ lockfile が変わらないのか](#なぜ-lockfile-が変わらないのか)）。差分が出たら第三者依存が動いた合図なので、`git add packages/*/package.json` で対象を絞り、差分の中身を確認してから判断する（[lockfile に差分が出た場合](#lockfile-に差分が出た場合)）。v1.20.4 以前の手順書は「lockfile も一緒に commit」となっていたため、古い手順の記憶で作業しないこと。
- **lockfile と package.json の不整合**: 依存を変更したのに lockfile を更新していないと、CI の `pnpm install --frozen-lockfile` が失敗する。手順 3 はこれをローカルで先に検出するためのもの。
- **部分公開後の再実行**: `publish:all` の途中で失敗し一部パッケージだけ公開された状態で Actions を rerun すると、公開済み version はスキップされ、未公開分のみが公開される（pnpm の標準挙動）。手動復旧する場合は `npm view <pkg> versions` で公開済みを確認し、未公開分だけ `pnpm --filter <pkg> publish --tag <dist-tag> --no-git-checks` する。
